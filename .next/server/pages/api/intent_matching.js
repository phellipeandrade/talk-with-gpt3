...
/* harmony export */   "fulfillIntent": () => (/* binding */ fulfillIntent)
/* harmony export */ });
/* harmony import */ var _time_zones_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(37);
/* harmony import */ var _conversationContext_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(38);
/* harmony import */ var _temporal__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(714);
/*
...
async function matchIntent(intentRequest, locale) {
    ...
    if (lcIntentReq.includes(" current ") || lcIntentReq.includes("currently") || lcIntentReq.includes(" now") || lcIntentReq.includes(" at the moment") || lcIntentReq.includes(" at this moment") || lcIntentReq.includes(" most recent") || lcIntentReq.includes("at the present time") || lcIntentReq.includes("at present") || lcIntentReq.includes("at this time") || lcIntentReq.includes(" yet") || lcIntentReq.includes(" latest ") || lcIntentReq.includes("at the present moment")) {
        return ASK_FOR_CURRENT_INFO_INTENT;
    }
//TODO: ADD RETURN?
}
async function fulfillIntent(intentRequest, lang, conversationText) {
    let today = new Date();
    ...
    if (intent == ASK_FOR_CURRENT_INFO_INTENT || intent == ASK_POPULATION_INTENT || intent == ASK_AGE_INTENT && intentArg.trim() != "" || intent == ASK_TEMPERATURE_INTENT || intent == ASK_WEATHER_INTENT && intentArg.trim() != "" || intent == ASK_NEWS_INTENT || intent == ASK_STOCK_PRICE_INTENT || intent == ASK_FULL_MOON_INTENT || intent == CALCULATE_INTENT || intent == ASK_DISTANCE_INTENT || intent == ASK_TRAVEL_TIME_INTENT || intent == ASK_BUSINESS_INTENT || intent == ASK_DATE_INTENT || intent == ASK_TIME_INTENT) {
        // Get answers from the web.
        retFulfillment = invokeSerpapi(intent, intentRequest, lang, conversationText);
    }
    return retFulfillment;
}
async function invokeSerpapi(intent, intentRequest, lang, conversationText) {
    // Compose a question from the conversation text so that the SERP API query
    // has enough context.
    let questionFromConversation = await (0,_conversationContext_js__WEBPACK_IMPORTED_MODULE_2__.composeQuestionFromConversation)(conversationText);
    console.log("===Conversation so far: " + conversationText);
    console.log("===Question from conversation: " + questionFromConversation);
    console.log("===Intent: " + intent);
    let retFulfillment = (0,_temporal__WEBPACK_IMPORTED_MODULE_1__.getTemporalStr)(questionFromConversation, lang);
...
var __webpack_exports__ = __webpack_require__.X(0, [37,714,38], () => (__webpack_exec__(760)));
